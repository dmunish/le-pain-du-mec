        ...
        <div className="max-w-7xl mx-auto p-4 sm:p-6">
          ...
          <GlassCard className="mb-6" variant="light">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4">
              <div>
                <label className="text-xs text-line-gray block mb-1">Population Size</label>
                <input
                  type="number"
                  ...
                  onChange={(e) => handleParamChange("populationSize", Number.parseInt(e.target.value))}
                />
              </div>
              <div>
                <label className="text-xs text-line-gray block mb-1">Initial Infected</label>
                <input
                  type="number"
                  className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                  value={simulationParams.initialInfected}
                  onChange={(e) => handleParamChange("initialInfected", Number.parseInt(e.target.value))}
                />
              </div>
              <div>
                <label className="text-xs text-line-gray block mb-1">R₀ Value</label>
                <input
                  type="number"
                  step="0.1"
                  className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                  value={simulationParams.r0}
                  onChange={(e) => handleParamChange("r0", Number.parseFloat(e.target.value))}
                />
              </div>
            </div>
            ...
            {showAdvanced && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4">
                <div>
                  <label className="text-xs text-line-gray block mb-1">Incubation Period (days)</label>
                  <input
                    type="number"
                    className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                    value={simulationParams.incubationPeriod}
                    onChange={(e) => handleParamChange("incubationPeriod", Number.parseInt(e.target.value))}
                  />
                </div>
                <div>
                  <label className="text-xs text-line-gray block mb-1">Recovery Time (days)</label>
                  <input
                    type="number"
                    className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                    value={simulationParams.recoveryTime}
                    onChange={(e) => handleParamChange("recoveryTime", Number.parseInt(e.target.value))}
                  />
                </div>
                <div>
                  <label className="text-xs text-line-gray block mb-1">Fatality Rate (%)</label>
                  <input
                    type="number"
                    step="0.1"
                    className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                    value={simulationParams.fatalityRate}
                    onChange={(e) => handleParamChange("fatalityRate", Number.parseFloat(e.target.value))}
                  />
                </div>
                <div>
                  <label className="text-xs text-line-gray block mb-1">Social Distancing (%)</label>
                  <input
                    type="number"
                    className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                    value={simulationParams.socialDistancing}
                    onChange={(e) => handleParamChange("socialDistancing", Number.parseInt(e.target.value))}
                  />
                </div>
                <div>
                  <label className="text-xs text-line-gray block mb-1">Mask Usage (%)</label>
                  <input
                    type="number"
                    className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                    value={simulationParams.maskUsage}
                    onChange={(e) => handleParamChange("maskUsage", Number.parseInt(e.target.value))}
                  />
                </div>
                <div>
                  <label className="text-xs text-line-gray block mb-1">Vaccination Rate (%)</label>
                  <input
                    type="number"
                    className="w-full bg-bg-dark/50 border border-line-gray/30 rounded-md px-3 py-2 text-white text-sm"
                    value={simulationParams.vaccinationRate}
                    onChange={(e) => handleParamChange("vaccinationRate", Number.parseInt(e.target.value))}
                  />
                </div>
              </div>
            )}
              ...
              {simulation.status === "running" && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                  ...
                  <div className="glass p-3 rounded-lg">
                    ...
                    <p className="text-white text-lg font-medium">{simulation.infectedAgents.toLocaleString()}</p>
                  </div>
                  <div className="glass p-3 rounded-lg">
